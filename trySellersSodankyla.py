...
class RTMOD(object):
  ...
  def sellersUfunc(self, mu, LAI, SC, pft, wv):
    check = np.isnan(mu)|np.isnan(LAI)|np.isnan(SC)|np.isnan(pft)|(SC==0)
    if check:
      return np.nan
    else:
      ...
      #soil reflectance
      self.t.soil_r=self.getGroundAlbedo(SC, wv)#SC
  ...
  def saveOutput(self):
    ...
    ds = xr.merge([LCds, LAIds, Snowds, Albedods]).to_netcdf(outfile, engine='scipy', format='NETCDF3_CLASSIC')
