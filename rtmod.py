...
class RTMOD(object):
  def __init__(self, rootDir, ALSfile, lidar, threshold):
    ...
    self.getLandCover()
    #self.getSummerLAI()
    self.getThisSummerLAI()
    self.loadSodankylaAlbedo()
...
if __name__=='__main__':
  ...
  rtmod.main()
  outList = sorted(glob.glob(os.path.join(rootDir, 'spartacusOut/*BSA_nir.nc')))
  df = pd.DataFrame()
  cnt = 1
  for outFile in outList:
    print(cnt, '/', len(outList), end='\r'); cnt+=1
    df = pd.concat([df, readSPARTACUS(outFile)], ignore_index=True)
  if 'als' in rtmod.lidar:
    df.to_pickle('/exports/csce/datastore/geos/users/s1503751/MODIS/sodankyla/rt.%s.%s.annuallai.pkl' % (rtmod.lidar, rtmod.alsThreshold))
  else:
    df.to_pickle('/exports/csce/datastore/geos/users/s1503751/MODIS/sodankyla/rt.%s.annuallai.pkl' % (rtmod.lidar))
